...
```
```{r}
height_df <- raster::as.data.frame(Lands_10m_above_sea_level)
```

```{r}
hist(height_df$layer)

qplot(height_df$layer, binwidth = 1)

```

```{r}
height_df_10m <- height_df %>% filter(layer <= 11.3)
```



```{r}
ggplot(height_df_10m, aes(x = layer)) +
...
ggplot(height_df_10m, aes(x = layer)) +
  ...
  xlim(c(0, 10)) +
  labs(
    title = "Total flooded area when sea level rise from 0 to 10m",
    x = "Rise in sea level (m)",
