...
img_frames <- paste0("VH_Top_uni_editedDTM/VHTop_", seq_len(FRAMES_NUM), ".png")

for (i in seq_len(FRAMES_NUM/2)) {
  message(paste(" - Generating image", i, "of", FRAMES_NUM))
  
  message(paste(" - Sea level of", format(round(WATERDEPTHS_UNISPEED[i] - 0.1, 2), nsmall = 2), "m"))
...
```{r}
# generate .png frame images
img_frames_rise <- paste0("VH_Top_uni_editedDTM/VHTop_", seq_len(FRAMES_NUM/2), ".png")

img_frame_sequence <- c(
  # rising
  img_frames_rise,
  # stop at top for 0.5 sec
  rep(paste0("VH_Top_uni_editedDTM/VHTop_", FRAMES_NUM/2, ".png"), 6),
  # dropping
  rev(img_frames_rise),
  # stop at bottom for 0.5 sec
  rep(paste0("VH_Top_uni_editedDTM/VHTop_", 1, ".png"), 6))

# write video
magick::image_write_video(magick::image_read(img_frame_sequence), 
                          path = "VH_sealevel_Top_uni_editedDTM.mp4", 
                          framerate = 12)
```
...
```{r}
# generate .png frame images
img_frames_rise <- paste0("VH_Axo_uni_editedDTM/VHTop_", seq_len(FRAMES_NUM/2), ".png")

img_frame_sequence <- c(
  # rising
  img_frames_rise,
  # stop at top for 0.5 sec
  rep(paste0("VH_Axo_uni_editedDTM/VHAxo_", FRAMES_NUM/2, ".png"), 6),
  # dropping
  rev(img_frames_rise),
  # stop at bottom for 0.5 sec
  rep(paste0("VH_Axo_uni_editedDTM/VHAxo__", 1, ".png"), 6))
